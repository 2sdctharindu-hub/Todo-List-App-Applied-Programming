...
6. Module & Function Breakdown
(i) main.py: The entry point that creates the GUI, sets up frames for active and completed tasks, and connects all functions. (Handled by: Nadeera Tharindu)
(ii) ui_components.py: Contains the visual elements and layout of the application. (Handled by: Nadeera Tharindu)
(iii) logic_module.py: Functions for managing tasks, adding/completing tasks, and grouping them by date. (Handled by: Md Ahmed)
(iv) storage_module.py: Functions for saving/loading tasks from tasks.txt to persist data. (Handled by: Md Ahmed)
